...
const NOTEFREQS = {
  ...
  export function getNames(rootNote, octave, scale){
    let rootIndex = ROOTNOTES.indexOf(rootNote) + (octave * 12)
    // find the index of the root note within the octave (0-11), 
    // then add 12 for each octave above zero the note is
    // this yields the exact index of the root note
    
    let indexArr = SCALESTEPS[scale].map((x, i) => { return x + rootIndex })
    // use the selected scale's array of steps-from-root to generate 
    // an array of all exact indices for the notes in the scale
    
    return indexArr.map(val => { return Object.keys(NOTEFREQS)[val] })
    // use index array to pull correct note names from data hash
  }
  
  export function getFreqs(rootNote, octave, scale){
    let rootIndex = ROOTNOTES.indexOf(rootNote) + (octave * 12)  
    let indexArr = SCALESTEPS[scale].map((x, i) => { return x + rootIndex })
    return indexArr.map(val => { return Object.values(NOTEFREQS)[val] })
    // same method, but pulling values from the data hash for frequencies     
  }
